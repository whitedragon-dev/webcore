...
var CONFIG = {
  ...
  MAX_HISTORY: 20, // Reduced for speed
  CORS_ORIGIN: '*'
};
...
'function renderMessages(e){chatContainer.innerHTML="";if(!e||0===e.length){var t=document.createElement("div");t.className="message assistant";t.innerHTML=\'<div class="label">Assistant</div><p>Start a conversation! Ask me anything.</p>\';chatContainer.appendChild(t);return}for(var n=0;n<e.length;n++){var r=e[n];addMessageDOM(r.role,r.content)}chatContainer.scrollTop=chatContainer.scrollHeight}' +
'function addMessageDOM(e,t){var n=document.createElement("div");n.className="message "+e;var r=document.createElement("div");r.className="label";r.textContent=e==="user"?"You":"Assistant";var a=document.createElement("div");if(e==="assistant")a.innerHTML=renderMarkdown(t);else a.textContent=t;n.appendChild(r);n.appendChild(a);chatContainer.appendChild(n);chatContainer.scrollTop=chatContainer.scrollHeight}' +
...
'function sendMessage(){var e=userInput.value.trim();if(!e||isProcessing)return;if(!currentConversationId){showError("Please create or select a conversation first");return}var t=modelSelect.value;addMessageDOM("user",e);userInput.value="";userInput.style.height="auto";isProcessing=true;sendBtn.disabled=true;showTyping(true);updateModelBadge(t);apiRequest("/api/chat","POST",{conversation_id:currentConversationId,prompt:e,model:t,temperature:0.7,max_tokens:1000}).then(function(t){if(t.success){addMessageDOM("assistant",t.response);conversationHistory=t.messages||[];loadConversations();chatSubtitle.textContent=conversationHistory.length+" messages";showError(null)}else{showError(t.error||"AI request failed")}}).catch(function(e){showError(e.message||"Error sending message")}).finally(function(){isProcessing=false;sendBtn.disabled=false;showTyping(false);userInput.focus()})}' +
'userInput.addEventListener("input",function(){userInput.style.height="auto";userInput.style.height=Math.min(userInput.scrollHeight,150)+"px"});userInput.addEventListener("keydown",function(e){if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMessage()}});sendBtn.addEventListener("click",sendMessage);newChatBtn.addEventListener("click",createNewConversation);refreshBtn.addEventListener("click",loadConversations);hamburgerBtn.addEventListener("click",function(){sidebar.classList.toggle("open")});document.addEventListener("click",function(e){if(window.innerWidth<=480&&!sidebar.contains(e.target)&&e.target!==hamburgerBtn){sidebar.classList.remove("open")}});modelSelect.addEventListener("change",function(){updateModelBadge(this.value)});updateModelBadge(modelSelect.value);loadConversations();userInput.focus();setTimeout(function(){var e=conversationList.querySelectorAll(".conversation-item");if(e.length>0)loadConversation(e[0].dataset.id)},300);' +
'<\/script></body></html>';
...
export default {
  async fetch(request, env) {
    ...
    // DELETE /api/conversations/:id - FIXED
    if (method === 'DELETE' && path.match(/^\/api\/conversations\/[^\/]+$/)) {
      try {
        var id = path.split('/').pop();
        
        // Delete messages first (ON DELETE CASCADE should handle this, but being explicit)
    ...
    if (method === 'POST' && path === '/api/chat') {
      try {
        ...
        // Get limited history (only last 20 messages for speed)
