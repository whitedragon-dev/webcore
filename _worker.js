...
export default {
  async fetch(request, env) {
    ...
    // GET /api/neurons - Get today's neuron usage
    ...
    // POST /api/chat - OPTIMIZED with accurate neuron tracking
    if (method === 'POST' && path === '/api/chat') {
      try {
        ...
        // Estimate neuron cost (rough estimate based on tokens)
        ...
        if (currentUsage + estimatedNeurons > CONFIG.DAILY_NEURON_LIMIT) {
          return Response.json({
            success: false,
            error: 'Daily neuron limit exceeded. You have ' + (CONFIG.DAILY_NEURON_LIMIT - currentUsage) + ' neurons remaining.'
          }, {
            status: 429,
            headers: { 'Access-Control-Allow-Origin': CONFIG.CORS_ORIGIN }
          });
        ...
        // Extract response and neuron usage
        var resultText = '';
        var neuronsUsed = 0;
        ...
        // Update neuron usage (accurate tracking from now on)
        var newUsage = currentUsage + neuronsUsed;
        var upsertStmt = env.DB.prepare(
          'INSERT INTO neuron_usage (date, used) VALUES (?, ?) ' +
